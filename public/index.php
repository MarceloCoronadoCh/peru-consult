...
// Parse the URI - compatible with proxy/subdirectory deployments
$requestUri = parse_url($_SERVER["REQUEST_URI"], PHP_URL_PATH);
$path = trim($requestUri, "/");

// Remove any index.php from path
...
try {
    ...
    if (count($segments) >= 2 && $segments[0] === "dni") {
    ...
    if (count($segments) >= 2 && $segments[0] === "ruc") {
    ...
    // Route not found
    errorResponse(
        "Endpoint no encontrado. Consulta /health para ver los endpoints disponibles.",
        404,
    );
} catch (Exception $e) {
